...
(function($){
	...
	// url validation regular expression was taken from jQuery url validation
		...
		popUpQr = popUpMenu.filter('[data="qr"]'),
		popUpMtf = popUpMenu.filter('[data="mtf"]'),
		expandButtons = $('.expand'),
		inputs = $('.popup-menu').filter(':input').filter('[type="text"]'),
		emptyFields, // test variable for verifing empty input fields
		inputValueTest = [],
		inputValueForm = [],
		dropDowns = $('.drop-down'),
		dropDownQr = dropDowns.filter('[data="qr"]'),
		dropDownMtf = dropDowns.filter('[data="mtf"]'),
		frameInputData = {
			qr: {	name: ['','',''],
					url: ['','','']
					},
	...
	webApp = {
	    		...
	    		// set tab from last hash or defualt
		    		...
		    		if(objectData !== null && objectData.lastHash.length>0){
		    			...
		    			if(objectData.qr.name.length>0 && objectData.mtf.name.length>0 ){
		    				if(!popUpQr.hasClass('hidden')){
	    						popUpQr.addClass('hidden');
		    				}  
		    				if(!popUpMtf.hasClass('hidden')){
	    						popUpMtf.addClass('hidden');
		    				}									
		    			} else {
	    	...
	    	$.ajax('/data/config.json', {
				success: function(response) {
					if(response.notification){
						...
						notification.innerHTML = response.notification;
						quickActions = response.quickActions;
						tabsList = response.tabsList;
						webApp.jsonContentToHTML('success');
					}
				},
				error: function(err){
	    ...
	    keyboardEvents: function(){
	    	...
	    	nav.onkeyup = function(e){
				...
				if($target.nodeName.toLowerCase() === 'a'){
					$target.onfocus = function(elm){
						if(!$($target.parentElement.parentElement.parentElement.parentElement).hasClass('hover')){
							for(var i = 0; i<navSection.length; i++){
								navSection.eq(i).removeClass('hover');
							}
							$($target.parentElement.parentElement.parentElement.parentElement).addClass('hover');
						}
					}
				}
	    		...
	    		if($target.nodeName.toLowerCase() === 'div' && UTILS.hasClass(target, 'menu-hint')){
	    			$target.onblur = function(elm){
						...
						$($target.parentElement.parentElement).addClass('hover');
					}
				}
	    	...
	    	tabs.onkeyup = function(e){
		    	...
		    	if (e.keyCode === 27){
		    		if(targetMatch.eq(0) === 'qr'){
		    			//console.log('esc clicked in qr');
		    			popUpQr.addClass('hidden');
		    		} 
		    		if(targetMatch.eq(0) === 'mtf'){
   		...
   		tabSelect: function(elm, elmHref){
			if(elm.nodeName.toLowerCase() === 'a' && elm.nodeType === 1){
		        ...
		        for(var i = 0; i<frames.length; i++){
					...
					if('#'+frames.eq(i).id === elmHref.match(re)[0] || '#'+frames.eq(i).id === location.hash){
						frames.eq(i).setAttribute('class', 'active');
						tabHash = '#'+frames.eq(i).id; // store temporary hash
						// set focus on first input here if the frame has popup-menu
						if(frames.eq(i).firstElementChild.lastElementChild.classList[0] === 'popup-menu'){
							frames.eq(i).firstElementChild.lastElementChild.children[0][1].focus();
						}
					}
				}
			}
	    ...
	    frameEvents: function(){
	    	...
	    	tabs.on('click', function(e){
		        ...
		        $(e).preventDefault();
		        var $target = $(e).target,
		        	parentElementOfTarget = $target.parent(),
			        ...
			        saveButton = function(){
			        	...
			        	if($target.className === 'popup-button' && $target.value === 'save'){
					    	...
					    	for(var i = 0; i< popUpMenu.length; i++){
					    		if(popUpMenu.eq(i).getAttribute('data') === $target.getAttribute('data')){
					    			...
					    			if(invalidInputs.length > 0){
					    				...
					    				if($target.getAttribute('data') === 'qr'){
					    					...
					    					for(var i=0; select.children.length>i; i++){
					    						if(select.children.eq(i).selected){
					    							frameInputData.lastHash = location.hash + '/' + select.children.eq(i).textContent;
					    						}
					    					}
			...
			tabs.on('change', function(e){
				...
				if($target.tagName.toLowerCase() === 'select'){
					...
					for(var i = 0; i<expandButtons.length; i++){
						if(expandButtons.eq(i).getAttribute('data') === $target.getAttribute('data')){
							expandButtons.eq(i).href = $target.value;
						}
					}
					// set the iframe src occording to the dropdown value
					for(var i = 0; i<iframeWindows.length; i++){
						if(iframeWindows.eq(i).getAttribute('data') === $target.getAttribute('data')){
					...
					function validatePlace(){
						if(validateForm()){
							...
							if(targetObj.formType === 'name'){
								...
								if($('#' + targetObj.formName + '-url' + targetObj.storePlace).value !== ''){
									// url is filled ans also name
									$('#' + targetObj.formName + '-name' + targetObj.storePlace).addClass('both');
									$('#' + targetObj.formName + '-url' + targetObj.storePlace).addClass('both');
									emptyFields = ''; // clear tester
									// check url has error - if true focus on it
									if($('#' + targetObj.formName + '-url' + targetObj.storePlace).parentElement.lastElementChild.classList.length<2){
									...
									$('#' + targetObj.formName + '-url' + targetObj.storePlace).focus();
									// show error message
									$('#' + targetObj.formName + '-url' + targetObj.storePlace).parentElement.lastElementChild
										.setAttribute('class',$('#' + targetObj.formName + '-url' + targetObj.storePlace).parentElement.lastElementChild.classList[0]);
									$('#' + targetObj.formName + '-url' + targetObj.storePlace).parentElement.lastElementChild.textContent = "Please fill in a URL";
								}
								if(targetObj.formData === '' && $('#' + targetObj.formName + '-url' + targetObj.storePlace).value === ''){
									// name and url are empty - remove url message
									$('#' + targetObj.formName + '-url' + targetObj.storePlace).parentElement.lastElementChild
										.setAttribute('class',$('#' + targetObj.formName + '-url' + targetObj.storePlace).parentElement.lastElementChild.classList[0] + " hidden");
								...
								} else if(targetObj.formData === '' && $('#' + targetObj.formName + '-url' + targetObj.storePlace).value !== '') {
									// name empty but url filled - show name error message
									$('#' + targetObj.formName + '-name' + targetObj.storePlace).parentElement.lastElementChild
										.setAttribute('class',$('#' + targetObj.formName + '-name' + targetObj.storePlace).parentElement.lastElementChild.classList[0]);
									$('#' + targetObj.formName + '-name' + targetObj.storePlace).parentElement.lastElementChild.textContent = "Please fill in a name";
									emptyFields = ''; // clear tester
								}
								...
								if($('#' + targetObj.formName + '-name' + targetObj.storePlace).value !== ''){
									// name is filled and also url
									$('#' + targetObj.formName + '-name' + targetObj.storePlace).addClass('both');
									$('#' + targetObj.formName + '-url' + targetObj.storePlace).addClass('both');
									emptyFields = ''; // clear tester
									// check name has error - if true focus on it
									if($('#' + targetObj.formName + '-name' + targetObj.storePlace).parentElement.lastElementChild.classList.length<2){
								...
								if(targetObj.formData === '' && $('#' + targetObj.formName + '-name' + targetObj.storePlace).value === ''){
									// name and url are empty - remove name message
									$('#' + targetObj.formName + '-name' + targetObj.storePlace).parentElement.lastElementChild
										.setAttribute('class',$('#' + targetObj.formName + '-name' + targetObj.storePlace).parentElement.lastElementChild.classList[0] + " hidden");
										// remove both url and name values from object
										//console.log('before: ' + frameInputData[targetObj.formName]['name'][targetObj.storePlace-1]);
										frameInputData[targetObj.formName]['name'][targetObj.storePlace-1] = '';
										//console.log('before: ' + frameInputData[targetObj.formName]['url'][targetObj.storePlace-1]);
								...
								} else if(targetObj.formData === '' && $('#' + targetObj.formName + '-name' + targetObj.storePlace).value !== '') {
...
//-----------------------//
	    ...
	    jsonContentToHTML: function(message){
	    	if(message === 'success'){
	    		//console.log('ajax loaded');
		    	for(var i = 0; i<navSection.length; i++){
		    		// loading data from json to the html: 
		    		// nav-section first paragraph text, + bg image + menu-hint paragraph text + menu data and anchors
		    		if(navSection.eq(i).firstElementChild.tagName.toLowerCase() === 'p'){
		    			navSection.eq(i).firstElementChild.innerHTML = quickActions.eq(i).label; 
		    			navSection.eq(i).style.backgroundImage = 'url("../img/' + quickActions.eq(i).icon + '.png")';
		    		}
		    		if(navSection.eq(i).lastElementChild.className === 'menu' 
		    			&& navSection.eq(i).lastElementChild.firstElementChild.className === 'menu-hint'){
		    			navSection.eq(i).lastElementChild.firstElementChild.firstElementChild.innerHTML = quickActions.eq(i).actionsLabel;
		    		}
		    		if(navSection.eq(i).querySelector('ul.action-list').className = 'action-list'){
		    			//console.log('ul selected');
		    			var actions =  quickActions.eq(i).actions,
		    				actionsUrl = [],
		    				actionsLabel = [],
		    				anchorArray = [];
		    				
		    			for(var j = 0; j<actions.length; j++){
		    				actionsLabel.push(actions[j].label);
		    				actionsUrl.push(actions[j].url);
		    			}
		    			for(var k = 0; k<actionsLabel.length; k++){
		    				var anchor = document.createElement('a'),
		    					li = document.createElement('li');
		    				anchor.href = actionsUrl[k];
		    				anchor.target = '_blank';
		    				anchor.setAttribute('tabindex', '0');
		    				anchor.textContent = actionsLabel[k];
		    				li.appendChild(anchor);
		    				navSection.eq(i).$('ul.action-list').appendChild(li);
		    			}
		    		}
		    	}
		    	// set defualt tabs and iframes
		    	for(var i=0; i<tabsList.length; i++){
		    		if(tabsList[i].options.url !== undefined){
		    			//console.log(tabsList.eq(i).options.url);
		    			expandButtons.eq(i).href = tabsList[i].options.url;
		    			iframes.eq(i).setAttribute('src', tabsList[i].options.url);
		    			//console.log(iframes.eq(i));
		    		}
		    	}
	    	}
	    }, // end of json Content To HTML function
		...
		setIframe: function(lastTab, lastUrl, lastQr, lastMtf){
	    	for(var i=0; iframeWindows.length>i; i++){
	    		...
	    		// [2] if lastFrame exists replace last index src with lastFrame src click
	    ...
	    setExpand: function(lastTab, lastUrl, lastQr, lastMtf){
	    	for(var i=0; expandButtons.length>i; i++){
	    		// [1] get url data from object and set both frames
	    		if(expandButtons.eq(i).getAttribute('data') === 'qr' && lastQr !== ''){
	    			expandButtons.eq(i).href = lastQr;
	    			expandButtons.eq(i).removeClass('hidden');
	    		} else if(expandButtons.eq(i).getAttribute('data') === 'mtf'  && lastMtf !== ''){
	    			expandButtons.eq(i).href = lastMtf;
	    			expandButtons.eq(i).removeClass('hidden');
	    		}
	    		// [2] if lastFrame exists replace last index src with lastFrame src lick
	    		if(expandButtons.eq(i).getAttribute('data') === lastTab.getAttribute('data')){
	    			expandButtons.eq(i).href = lastUrl;
	    		}
	    	}
	    }, // end of set iframe
	    ...
	    inputsNDropdowns: function(restoredData){
	    	...
	    	for(var i = 0; inputs.length>i; i++){
	    		...
	    		targetObj.formName = inputs.eq(i).id.match(matchRe)[0]; // qr or mtf
				targetObj.formType = inputs.eq(i).id.match(matchRe)[1]; // name or url
				targetObj.storePlace = inputs.eq(i).id.match(matchRe)[2]; // place in form 1,2,3?
				...
				inputs.eq(i).value = targetObj.formData;
	    ...
	    expendAndIframeSettings: function(formName, url){
			...
			for(var j = 0; j < expandButtons.length; j++){
				...
				if(expandButtons[j].getAttribute('data') === formName){
					...
					expandButtons[j].removeClass('hidden');
			...
			if(formName){
				...
				if(formName === 'qr'){
					...
					for(var i = dropDownQr.children.length-1; i >= 0; i--){
						if(dropDownQr.children.eq(i).textContent !== ''){
							dropDownQr.children.eq(i).setAttribute('selected', 'selected');
							webApp.expendAndIframeSettings('qr', dropDownQr.children.eq(i).value);
							// set hash to form name and selected report
							if(dataObject.lastHash === false){
								location.hash = tabHash + '/' + dropDownQr.children.eq(i).textContent;
				...
				if(formName === 'mtf'){
					...
					for(var i = dropDownMtf.children.length-1; i >= 0; i--){
						if(dropDownMtf.children.eq(i).textContent !== ''){
							dropDownMtf.children.eq(i).setAttribute('selected', 'selected');
							webApp.expendAndIframeSettings('mtf', dropDownMtf.children.eq(i).value);
							// set hash to form name and selected report
							if(dataObject.lastHash === false){
								location.hash = tabHash + '/' + dropDownQr.children.eq(i).textContent;
								frameInputData.lastHash = location.hash; // store hash to object
		...
		dropDownBuildProccess: function(dropDownElm, form, inc, dataObject){
			...
			optionElm.appendTo(dropDownElm);
		...
		inputsClearTest: function(test){
			...
			if(test === 'empty'){
				for(var i = 0; i<inputs.length; i++){
					inputValueTest.push(inputs.eq(i).value); // insert all values to array
					inputValueForm.push(inputs.eq(i).id.match(matchRe)[0]); // insert form name to array
				}
				//console.log(inputValueTest);
				//console.log(inputValueForm);
				for(var i = 0; i<inputValueTest.length; i++){
					if(inputValueTest.eq(i) !== ''){
						//console.log(inputValueForm.eq(i) + ' form not empty');
						if(inputValueForm.eq(i) === 'qr'){
							qrCount++;
						} else if(inputValueForm.eq(i) === 'mtf'){
							mtfCount++;
						}
					}
				...
				*****/
				...
				if(qrCount === 0){
					...
					$('.expand[data="qr"]').addClass('hidden');
					$('.expand[data="mtf"]').href = '';
					$('iframe[data="qr"]').src = '';
				...
				// case 2 - mtf fields all empty
					...
					$('.expand[data="mtf"]').addClass('hidden');
					$('.expand[data="mtf"]').href = '';
					$('iframe[data="mtd"]').src = '';
